function mlx_embeddings_quantize_hf_model --wraps=mlx_lm.convert -a hf_path -a q_bits -a q_mode
    if test -z "$hf_path"
        echo "Usage: mlx_embeddings_quantize_hf_model <hf_path> [q_mode] [q_bits]" >&2
        return 1
    end
