...
return {
  entry = function(st, job)
    ...
    if action == "fix" then
      ...
      if not cx.active.current.cwd:starts_with(st.root) then
        ---@diagnostic disable-next-line: undefined-global
        ya.emit("plugin", { "pane", "hide-parent" })
        ---@diagnostic disable-next-line: undefined-global
        ya.emit("cd", { tostring(st.root) })
        ---@diagnostic disable-next-line: undefined-global
  ...
  setup = function(st, opts)
    opts = opts or {}
    opts.root = opts.root or os.getenv("YAZI_BOUNDARY_ROOT_DIR")

    if opts.root ~= nil and opts.root ~= "" then
      ---@diagnostic disable-next-line: undefined-global
      ya.emit("plugin", { "boundary", "set-root" .. " " .. tostring(opts.root) })
    end

    ---@diagnostic disable-next-line: undefined-global
    ya.emit("plugin", { "boundary", "enable-auto-fix" })
  end,
}
