...
return {
  ...
  entry = function(st, job)
    ...
    if action == "toggle-parent" then
      ...
      local is_parent_visible = ((st.parent == rt.mgr.ratio[1] and 0) or rt.mgr.ratio[1]) > 0
    ...
    elseif action == "show-parent" then
      st.parent = rt.mgr.ratio[1]
      ---@diagnostic disable-next-line: undefined-global
      st.current = st.old_current or rt.mgr.ratio[2]
      st.old_current = nil
      ---@diagnostic disable-next-line: undefined-global
      st.preview = st.old_preview or rt.mgr.ratio[3]
      st.old_preview = nil
      should_resize = true
