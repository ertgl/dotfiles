...
dotfiles::config::update() {
  ...
  local sudo_flag="${DOTFILES_SUDO:-0}"
  ...
  do
    ...
    if ! _maybe_sudo mkdir -p "$(dirname "${dest_file}")";
    then
    ...
    if _maybe_sudo ln -fs "${src_file}" "${dest_file}";
    then
  ...
  do
    if _maybe_sudo rm "${broken_symlink}";
    then
      ...
      then
        if _maybe_sudo rmdir "${parent_dir}";
        then
...
dotfiles::config::sudo-update() {
  DOTFILES_SUDO=true dotfiles::config::update "$@"
}
