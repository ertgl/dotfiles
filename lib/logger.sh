if [[ $(type -t dotfiles::lib::logger::loaded) == function ]];
then
  return
fi
...
source "${DOTFILES_LIB_DIR}/color.sh"
...
dotfiles::logger::color-for-message() {
  ...
  case "$level" in
    "$DOTFILES_LOG_LEVEL_SUCCESS")
      echo dotfiles::color::green
      ;;
    "$DOTFILES_LOG_LEVEL_INFO")
      echo dotfiles::color::blue
      ;;
    "$DOTFILES_LOG_LEVEL_DEBUG")
      echo dotfiles::color::cyan
      ;;
    "$DOTFILES_LOG_LEVEL_WARNING")
      echo dotfiles::color::yellow
      ;;
    "$DOTFILES_LOG_LEVEL_ERROR")
      echo dotfiles::color::red
      ;;
    *)
      echo dotfiles::color::reset
      ;;
    esac
}
...
dotfiles::logger::gen-prefix() {
  ...
  echo -e "$(
    dotfiles::color::grey "${bash_source}:${BASH_LINENO[3]}"
  ):"
}
...
dotfiles::logger::delegated::gen-prefix() {
  ...
  echo -e "$(
    dotfiles::color::grey "${bash_source}:${BASH_LINENO[4]}"
  ):"
}
